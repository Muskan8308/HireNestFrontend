...
public class ExceptionControllerAdvice {
	...
	@ExceptionHandler(JobPortalExceptions.class)
	public ResponseEntity<ErrorInfo> generalException(JobPortalExceptions e)
	{
	...
	public ResponseEntity<ErrorInfo> validatorExceptionHandler(Exception e)
	{
		...
		if(e instanceof MethodArgumentNotValidException manvExcep)
		{
			msg = manvExcep.getAllErrors().stream().map(ObjectError::getDefaultMessage).collect(Collectors.joining(", "));
		}
		else
		{
			ConstraintViolationException cvExcep = (ConstraintViolationException) e;   // type casted "e"
			msg = cvExcep.getConstraintViolations().stream().map(ConstraintViolation::getMessage).collect(Collectors.joining(", "));
